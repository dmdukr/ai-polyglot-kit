...
🎤 Audio capture (microphone)
    ↓
🔇 VAD silence detection → split into chunks
    ↓
☁️ STT Provider (#1 → #2 → #3 on quota exhaust) → raw transcription
    ↓
🧠 LLM Normalization (using saved user profile: corrections, terms, rules)
   • removes filler words (ну, эм, like, uh...)
   • fixes Whisper recognition errors
   • restores punctuation, capitalization
   • formats numbers and dates
   • preserves language (never translates UK↔RU↔EN)
    ↓
⌨️ Result typed into active window
```

### User workflow

1. **Place cursor** in any text input field (browser, Word, Telegram, IDE — anywhere)
2. **Start recording**:
   - **Hold mode**: hold the hotkey (default: `F12`) and speak
   - **Toggle mode**: press hotkey once to start, speak, press again to stop
3. **Dictate** your text — you can speak in paragraphs, the app splits on pauses
4. **Stop recording**: release the key (Hold mode) or press hotkey again (Toggle mode)
5. **Wait** — the app processes audio and types the corrected text into your cursor position
6. **Review** the inserted text — if the AI made mistakes, correct them manually
7. **Teach the app** — double-tap the hotkey (the same key used for your active recording mode). The app captures your edits, compares them with its output, and saves the corrections. Next time, it won't make the same mistakes.

> **Self-learning**: Over time, the app builds a profile of your speech patterns, preferred terms, and common corrections. The more you use feedback, the more accurate it becomes.

### Quick Translate

1. **Select text** in any application
2. **Press Ctrl+C twice** quickly (within 0.5 seconds)
3. A translation overlay appears with the result (DeepL or LLM)
...
| Action | How |
|--------|-----|
| **Record (Hold)** | Hold `F12` → speak → release |
| **Record (Toggle)** | Press `F12` → speak → press `F12` again |
| **Feedback** | After text is typed, edit mistakes, then double-tap `F12` |
| **Translate** | Select text → `Ctrl+C` twice quickly |
...
🎤 Запис з мікрофону
    ↓
🔇 VAD визначення тиші → розбиття на фрагменти
    ↓
☁️ STT провайдер (#1 → #2 → #3 при вичерпанні ліміту) → сирий текст
    ↓
🧠 LLM нормалізація (з урахуванням збереженого профілю: виправлення, терміни, правила)
   • видаляє слова-паразити (ну, ем, like, uh...)
   • виправляє помилки розпізнавання Whisper
   • відновлює пунктуацію, великі літери
   • форматує числа та дати
   • зберігає мову (ніколи не перекладає UK↔RU↔EN)
    ↓
⌨️ Результат вводиться в активне вікно
```

### Робочий процес користувача

1. **Поставте курсор** у будь-яке поле вводу (браузер, Word, Telegram, IDE — де завгодно)
2. **Почніть запис**:
   - **Режим утримання**: тримайте гарячу клавішу (за замовч.: `F12`) і говоріть
   - **Режим перемикача**: натисніть клавішу для старту, говоріть, натисніть знову для зупинки
3. **Диктуйте** текст — можна говорити абзацами, програма розбиває по паузах
4. **Зупиніть запис**: відпустіть клавішу (Утримання) або натисніть знову (Перемикач)
5. **Зачекайте** — програма обробить аудіо та введе виправлений текст у позицію курсору
6. **Перегляньте** вставлений текст — якщо AI зробив помилки, виправте їх вручну
7. **Навчіть програму** — двічі натисніть гарячу клавішу (ту саму, що використовується для активного режиму запису). Програма захопить ваші правки, порівняє зі своїм результатом і збереже виправлення. Наступного разу вона не зробить тих самих помилок.

> **Самонавчання**: З часом програма формує профіль вашого мовлення, улюблених термінів та типових виправлень. Чим більше ви використовуєте зворотній зв'язок, тим точнішою вона стає.

### Швидкий переклад

1. **Виділіть текст** у будь-якому додатку
2. **Натисніть Ctrl+C двічі** швидко (протягом 0.5 секунди)
3. З'явиться вікно перекладу з результатом (DeepL або LLM)
4. Натисніть **Copy** або **Replace** для використання перекладу

## Короткий довідник

| Дія | Як |
|-----|-----|
| **Запис (Утримання)** | Тримайте `F12` → говоріть → відпустіть |
| **Запис (Перемикач)** | Натисніть `F12` → говоріть → натисніть `F12` знову |
| **Зворотній зв'язок** | Після вставки тексту виправте помилки, потім двічі натисніть `F12` |
| **Переклад** | Виділіть текст → `Ctrl+C` двічі швидко |
